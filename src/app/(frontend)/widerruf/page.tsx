...
export default async function WiderrufPage() {
  ...
  const cms = siteTexts?.legal?.widerruf || {}

  const pageTitle = cms?.title || 'Widerrufsrecht & Widerrufsformular'
  const subtitle = cms?.subtitle || 'Stand: Mai 2026'

  const introContent = cms?.introContent as DefaultTypedEditorState | undefined
  const exclusionContent = cms?.exclusionContent as DefaultTypedEditorState | undefined
  const consequencesContent = cms?.consequencesContent as DefaultTypedEditorState | undefined
  const formIntroContent = cms?.formIntroContent as DefaultTypedEditorState | undefined
  const templateContent = cms?.templateContent as DefaultTypedEditorState | undefined

  const introHTML =
    introContent?.root?.children?.length ? convertLexicalToHTML({ data: introContent }) : null
  const exclusionHTML =
    exclusionContent?.root?.children?.length ? convertLexicalToHTML({ data: exclusionContent }) : null
  const consequencesHTML =
    consequencesContent?.root?.children?.length ? convertLexicalToHTML({ data: consequencesContent }) : null
  const formIntroHTML =
    formIntroContent?.root?.children?.length ? convertLexicalToHTML({ data: formIntroContent }) : null
  const templateHTML =
    templateContent?.root?.children?.length ? convertLexicalToHTML({ data: templateContent }) : null
  ...
  return (
    <main className={styles.page}>
      <div className={styles.container}>
        ...
        {introHTML ? (
          <section className={styles.section} dangerouslySetInnerHTML={{ __html: introHTML }} />
        ) : (
        ...
        {exclusionHTML ? (
          <section className={styles.section} dangerouslySetInnerHTML={{ __html: exclusionHTML }} />
        ) : (
          <section className={styles.section}>
        ...
        {consequencesHTML ? (
          <section className={styles.section} dangerouslySetInnerHTML={{ __html: consequencesHTML }} />
        ) : (
          <section className={styles.section}>
        ...
        <section className={styles.section}>
          ...
          {formIntroHTML ? (
            <div dangerouslySetInnerHTML={{ __html: formIntroHTML }} />
          ) : (
            <p>
        ...
        {templateHTML ? (
          <section className={styles.section} dangerouslySetInnerHTML={{ __html: templateHTML }} />
        ) : (
          <section className={styles.section}>
