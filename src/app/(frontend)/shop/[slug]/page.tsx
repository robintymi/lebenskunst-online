...
export default async function ShopItemDetailPage({ params }: Props) {
  ...
  const categoryIds = (item.categories || []).map((category: any) =>
    ...
    <>
          ...
          <div className={styles.info}>
            ...
            {isEvent && item.eventDetails && (
              <div className={styles.infoGrid}>
                ...
                {spotsLeft !== null && (
                  <div className={styles.infoItem}>
                    <strong>Verfuegbarkeit</strong>
                    <span>{spotsLeft > 0 ? `${spotsLeft} Plaetze frei` : 'Ausgebucht'}</span>
                  </div>
                )}
              </div>
            )}
      ...
      <section className="section" style={{ paddingTop: 0 }}>
        <div className="container">
          <RecommendedItems
            allItems={allItems.map((recommendedItem: any) => ({
              id: recommendedItem.id,
              title: recommendedItem.title,
              slug: recommendedItem.slug,
              itemType: recommendedItem.itemType,
              categoryIds: (recommendedItem.categories || []).map((category: any) =>
                typeof category === 'string' ? category : category.id,
              ),
              featured: recommendedItem.featured,
              pricing: recommendedItem.pricing,
              image:
                recommendedItem.image && typeof recommendedItem.image !== 'string'
                  ? recommendedItem.image
                  : undefined,
              shortDescription: recommendedItem.shortDescription,
            }))}
